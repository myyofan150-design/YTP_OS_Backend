...
export async function createLeadMeta(req: Request, res: Response): Promise<void> {
  try {
    ...
    const maxRow = await q<RowDataPacket>(
      "SELECT COALESCE(MAX(sort_order), 0) AS maxOrder FROM lead_meta_options WHERE type = ?",
      [type]
    );
    ...
    const rows = await q<RowDataPacket>(
      `SELECT ${META_SEL} FROM lead_meta_options WHERE id = ?`,
      [result.insertId]
    );
...
export async function updateLeadMeta(req: Request, res: Response): Promise<void> {
  try {
    ...
    const rows = await q<RowDataPacket>(
      `SELECT ${META_SEL} FROM lead_meta_options WHERE id = ?`, [metaId]
    );
    const meta = rowToMeta(rows[0]);
...
export async function deleteLeadMeta(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (usageCount > 0) {
      res.status(400).json({ success: false, message: `In use by ${usageCount} lead${usageCount !== 1 ? "s" : ""}` });
      return;
    }

    await run("DELETE FROM lead_meta_options WHERE id = ?", [metaId]);
    await logActivity(req.user!.id, "lead_meta.deleted", "lead_meta", metaId, { label: existing[0]["label"] }, undefined, req.ip);
